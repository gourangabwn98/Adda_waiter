...
export default function LoginPage() {
  ...
  // ✅ Step 1: Validate phone against active chefs in DB
  const validateChefPhone = async (phoneNumber) => {
    try {
      const res = await fetch(`${API_BASE}/admin/chefs?status=Active`);
      if (!res.ok) throw new Error("Failed to fetch chefs");
      const data = await res.json();

      // data could be array directly or { chefs: [...] } — adjust to your API shape
      const chefs = Array.isArray(data) ? data : data.chefs || [];

      const activeChef = chefs.find(
        (chef) =>
          chef.phone === phoneNumber && chef.status === "Active"
      );

      return activeChef || null;
    } catch (err) {
      console.error("Chef validation error:", err);
      throw err;
    }
  };
  ...
  const handle = async () => {
    ...
    try {
      ...
      // ✅ Step 2: Check if phone belongs to an active chef
      const chef = await validateChefPhone(phone);
      if (!chef) {
        toast.error("This number is not registered as an active chef.");
        setLoading(false);
        return;
      }

      // ✅ Auto-fill name from DB if not entered
      if (!name && chef.name) {
        // Optionally prefill — or just proceed
      }
      ...
      window.confirmationResult = confirmationResult;
      sessionStorage.setItem("otpPhone", phone);
      sessionStorage.setItem("otpName", name || chef.name); // use DB name as fallback
      nav("/otp");
      ...
      if (e.code === "auth/too-many-requests") {
        ...
        toast.error("Invalid phone number.");
      } else if (e.message === "Failed to fetch chefs") {
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <TopBar
        ...
        right={
          <span
            onClick={handleSkip}
            style={{ color: pink, cursor: "pointer", fontSize: 13, fontWeight: 600, padding: "4px 8px" }}
          >
            Skip
          </span>
        }
      ...
      <div style={{ flex: 1, padding: 24, display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <PinkBtn onClick={handle} disabled={loading}>
          {loading ? "Verifying…" : "Next & Verify Phone Number"}
        </PinkBtn>
