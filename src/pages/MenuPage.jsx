...
export default function MenuPage() {
  ...
  const addItem = useCallback((item) => {
    // ← guard: never let an unavailable item enter the cart
  ...
  const renderItem = (item) => {
    ...
    const cc = catColor(item.category);
    const cartItem = cart.find((c) => c._id === item._id);
    const isAvailable = item.isAvailable !== false; // ← key check (treat undefined/true as available)
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            <span
              style={{
                background: isAvailable ? cc.bg : "#eee",
                color: isAvailable ? cc.color : "#999",
                fontSize: 9,
                padding: "2px 7px",
                borderRadius: 10,
                fontWeight: 700,
