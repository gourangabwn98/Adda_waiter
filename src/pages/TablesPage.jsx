...
// Merge multiple orders' items into one flat list, combining duplicate names
...
// ── Bill Print ────────────────────────────────────────────────────────────────
function printBill({ table, mergedItems, subtotal, tax, total, invoiceId }) {
  let el = document.getElementById("bill-print-area");
  if (!el) {
    el = document.createElement("div");
    el.id = "bill-print-area";
  ...
  const rows = mergedItems
    .map(
      ...
      <tr>
        ...
        <td style="text-align:center;padding:5px 4px;">${i.qty}</td>
  ...
  el.innerHTML = `
    <div style="font-family:monospace;max-width:300px;margin:0 auto;padding:20px 10px;font-size:13px;">
      <div style="text-align:center;margin-bottom:12px;">
        <div style="font-size:22px;font-weight:800;color:#e91e8c;">আড্ডা</div>
        <div style="font-size:11px;letter-spacing:3px;color:#888;">ADDA CAFE</div>
        <div style="border-bottom:1px dashed #ccc;margin:10px 0;"></div>
        <div style="font-size:15px;font-weight:700;">TABLE ${table.tableNo} · BILL</div>
        <div style="font-size:11px;color:#888;margin-top:2px;">${new Date().toLocaleString()}</div>
        ${invoiceId ? `<div style="font-size:10px;color:#ccc;margin-top:2px;">INV …${invoiceId.slice(-8)}</div>` : ""}
      </div>
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
        <span>Subtotal</span><span>₹${Math.round(subtotal).toLocaleString()}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:4px;color:#888;">
        <span>GST (18%)</span><span>₹${Math.round(tax).toLocaleString()}</span>
      </div>
      <div style="border-bottom:1px dashed #ccc;margin:10px 0;"></div>
      ...
      <div style="border-bottom:1px dashed #ccc;margin:10px 0;"></div>
      <div style="text-align:center;color:#888;font-size:11px;line-height:1.8;margin-top:10px;">
        Thank you for dining with us!<br/>Visit again · আড্ডা Cafe
      </div>
    </div>`;
  window.print();
}
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: "#aaa", marginTop: 2 }}>
                  {date}
                  {o.user?.name && (
                    <span style={{ marginLeft: 6 }}>· {o.user.name}</span>
                  )}
                </div>
              </div>
                ...
                {o.items?.map((item, i) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "4px 0",
                      borderBottom:
                        i < o.items.length - 1 ? "1px solid #f0f0f0" : "none",
                    }}
                  >
                    <span>
                      <span
...
// ── Table Popup ───────────────────────────────────────────────────────────────
// orders = ALL active orders for this table (array)
function TablePopup({ table, orders, invoice, onClose, onRefresh }) {
  ...
  // For multi-order: "can deliver" if ANY order is not yet delivered
  const canDeliver = orders.some((o) =>
    ["Placed", "Preparing", "Ready"].includes(o.status)
  );
  // Can print bill only when ALL active orders are Delivered
  const canBill =
    orders.length > 0 &&
    orders.every((o) => o.status === "Delivered");

  // Dominant status for the header badge (worst = earliest in flow)
  const STATUS_PRIORITY = ["Placed", "Preparing", "Ready", "Delivered"];
  const dominantStatus =
    orders.length === 0
      ? "Empty"
      : STATUS_PRIORITY.find((st) => orders.some((o) => o.status === st)) ||
        orders[0].status;
  ...
  // Mark ALL non-delivered active orders as Delivered
  const handleDeliverAll = async () => {
    ...
    try {
      ...
      await Promise.all(
        toDeliver.map((o) => updateOrderStatus(o._id, "Delivered"))
      );
      toast.success(
        toDeliver.length > 1
          ? `${toDeliver.length} orders marked Delivered!`
      ...
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            <span
              style={{
                ...
                fontSize: 11,
                fontWeight: 600,
                background: isFree ? "#f0f0f0" : s.bg,
                color: isFree ? "#999" : s.tc,
                border: `1.5px solid ${isFree ? "#ddd" : s.border}`,
              }}
            >
        ...
        {tab === "order" && !isFree && (
          <div style={{ padding: "0 20px" }}>
            ...
            {/* Per-order breakdown if multiple */}
            {orders.length > 1 && (
            ...
            {canDeliver && (
              <button
                ...
                disabled={delivering}
                style={{ background: GREEN, color: WHITE, marginBottom: 10 }}
              >
                {delivering ? (
                  <>
                    <span
        ...
        {isFree && (
          <div style={{ padding: "0 20px" }}>
            ...
            >
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: PINK_DARK }}>
                  Table is Free
                </div>
                <div style={{ fontSize: 12, color: "#993556", marginTop: 3 }}>
        ...
        {/* History */}
        {/* {(tab === "history" || isFree) && (
          <HistoryTab tableNo={table.tableNo} />
        )} */}
      </div>
    </div>
  );
...
const StatCard = ({ label, val, color }) => (
  ...
  >
    ...
    <div style={{ fontSize: 11, color: "#aaa", marginTop: 2, fontWeight: 500 }}>
      {label}
    </div>
  </div>
);
...
const TableTile = ({ table, orders, invoice, onClick }) => {
  ...
  const badgeBg =
    ...
    dominantStatus === "Delivered" ? "#c8f7d6" :
    "#f0f0f0";
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          fontSize: 11,
          color: isPending ? "#e57373" : isInactive ? "#bbb" : s.tc,
          marginTop: 2,
        }}
      >
        {table.seats} seats
      ...
      <div style={{ marginTop: 8 }}>
        <span
          style={{
            ...
            fontSize: 10,
            fontWeight: 600,
            background: isPending ? "#ffcdd2" : isInactive ? "#eee" : badgeBg,
...
export default function WaiterTablesPage() {
  ...
  // tableOrdersMap: tableNo → array of ALL active orders
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      // Group ALL active dining orders by tableNo (array per table)
      const oMap = {};
      orders
        .filter(
          (o) =>
      ...
      // Sort each table's orders oldest→newest
      ...
      // Build invoice map against any order in each table's group
      const iMap = {};
      invoices.forEach((inv) => {
        const invOrderIds = inv.orders?.map(String) || [];
        for (const [tableNo, tableOrders] of Object.entries(oMap)) {
          const matched = tableOrders.some((o) =>
            invOrderIds.includes(String(o._id))
          );
          if (matched) {
            iMap[Number(tableNo)] = {
              ...inv,
  ...
  const occupied = activeTables.filter(
    (t) => (tableOrdersMap[t.tableNo] || []).length > 0
  ).length;
  ...
  if (loading)
    return (
      <div
        className="wt-root"
        style={{ textAlign: "center", padding: "100px 20px", color: "#ccc" }}
      >
        <div
          className="spinner"
          style={{ width: 32, height: 32, borderWidth: 3, margin: "0 auto 14px" }}
        />
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        >
          {[
            ...
            <div
              key={l.label}
              style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "#888" }}
            >
              <div
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {tables.map((t) => (
              <TableTile
                key={t.tableNo}
                table={t}
                orders={tableOrdersMap[t.tableNo] || []}
